			...
			if(this.state.userToken) {
				...
				.then(res => {
					...
					if(res.data.length !== 0) {
						...
						window.localStorage.setItem('secondOrderList', JSON.stringify(res.data));
	...
	render() {
		...
		let orderPage;
	    	if(firstState === 'true')
	    	{
		        ...
		        <List>
		        	<Divider />
			        {this.state.orders.map(d => (<div> <ListItem data-id={d.id} onClick={this.handleChange} key={d.id} button>
		    	...
		    	orderPage = <div>
		         ...
		         <List>
		         	<Divider />
			         {this.state.secondOrderList.map(d => (<div> <ListItem data-id={d.id} onClick={this.handleChange2} key={d.id} button>
			         	<ListItemText primary={"Price: " + d.total_price + "\tAddress: " + d.address}/>
			         </ListItem>
			         <Divider />
			         </div>))}
